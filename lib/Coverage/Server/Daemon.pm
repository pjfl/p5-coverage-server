package Coverage::Server::Daemon;
...
has '+config_class' => default => 'Coverage::Server::Config';
...
my $_build_daemon_control = sub {
   ...
   my $args   = {
      ...
      lsb_stop     => '$syslog',
      lsb_sdesc    => 'Coverage Server',
      lsb_desc     => 'Manages the Coverage Server daemons',
      path         => $conf->pathname,
...
Coverage::Server::Daemon - Background process control for the coverage server

=head1 Synopsis

   use Coverage::Server::Daemon;

   exit Coverage::Server::Daemon->new_with_options
      ( appclass => 'Coverage::Server' )->run;

=head1 Description

Manages the coverage web server process. The command line wrapper for this
...
Defaults to C<Coverage::Server::Config> the name of the applications
configuration class
