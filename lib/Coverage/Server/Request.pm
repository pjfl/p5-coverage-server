package Coverage::Server::Request;

use namespace::autoclean;

use Coverage::Server::Functions qw( extract_lang new_uri );
use Coverage::Server::Session;
...
my $_get_defined_value = sub {
   ...
   $v = $_defined_or_throw->( $name, $v, $opts );
...
my $_build__base = sub {
   my $self = shift; return $self->scheme.'://'.$self->host.$self->script.'/';
};
...
my $_build_uri = sub {
   my $self = shift; return new_uri $self->_base.$self->path, $self->scheme;
};
...
has 'session'        => is => 'lazy', isa => Object, builder => sub {
   Coverage::Server::Session->new( builder => $_[ 0 ]->_usul,
                                   env     => $_[ 0 ]->_env ) },
...
Coverage::Server::Request - Represents the request sent from the client to the server

=head1 Synopsis

   use Coverage::Server::Request;

   my $req = Coverage::Server::Request->new( $class_usul_ref, $model_moniker, @args );
...
Stores the user preferences. An instance of L<Coverage::Server::Session>
...
=head2 C<args_params>
   ...
   $code_ref = $self->args_params;

Returns a code reference which when called with an integer index returns
the args parameter value after first scrubbing it of "dodgy" characters. Throws
